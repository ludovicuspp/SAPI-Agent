...
from api.deps import get_db, get_current_user, get_ws_user
...
async def upload_boletin(
    ...
    conn.commit()
    db.user_log_action(conn, user.id, f"subir_boletin:{file.filename}")
...
async def ws_progress(
    ...
    Autenticado por query param `token` (JWT). Si el token falta/ es inválido
    o el usuario no tiene acceso al boletín (no es suyo y no es admin), la
    conexión se cierra sin aceptar.
    """
    if user is None:
    ...
    try:
        while True:
            ...
            signature = (
                ...
                b.processing_batch,
                b.hermes_progress_step,
                b.hermes_progress_current_page,
                b.hermes_progress_total_pages,
            )
            if signature != last_signature:
                await websocket.send_json({
                    ...
                    "hermes_progress_step": b.hermes_progress_step,
                    "hermes_progress_current_page": b.hermes_progress_current_page,
                    "hermes_progress_total_pages": b.hermes_progress_total_pages,
                    "hermes_processed_at": b.hermes_processed_at,
